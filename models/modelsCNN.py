...
#         self.bn_mean = nn.BatchNorm1d(fc2)
    ...
    def forward(self, x):
        z= self.encode(x)
        res = self.decode(z)
        return res, z, None, None
    
class CNN_VAE_small(nn.Module):
    def __init__(self, kernel, stride, padding, nc, hidden1, hidden2, hidden3, fc1,fc2):
        super(CNN_VAE_small, self).__init__()
...
#         self.bn_mean = nn.BatchNorm1d(fc2)
    ...
    def decode(self, z):
        h = self.fcD1(z)
        h = h.view(-1, self.hidden3, int(np.sqrt(self.fc1/self.hidden3)), int(np.sqrt(self.fc1/self.hidden3)))
        return self.decoder(h)
